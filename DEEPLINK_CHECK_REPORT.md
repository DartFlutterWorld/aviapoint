...
**Настроено:**
- ✅ Android Universal Links (intent-filter для https/http avia-point.com)
- ✅ Android Custom Scheme (aviapoint://)
- ✅ iOS Custom Scheme
- ✅ iOS Universal Links (entitlements)
- ✅ AutoRoute с defaultRouteParser — диплинки приходят от платформы в Router, парсер строит стек роутов
- ✅ Правильные пути в роутере (on-the-way/:id, market/:id, blog/:id, news/:id и т.д.)

**Почему на Android могут не открываться ссылки:**

1. **Ссылки https://avia-point.com/...**  
   Без файла **assetlinks.json** на сервере Android не считает приложение владельцем домена и открывает ссылку в браузере, а не в приложении. Intent в приложение при этом не приходит.  
   Нужно выложить `https://avia-point.com/.well-known/assetlinks.json` (см. пример выше в отчёте) и указать `package_name: ru.dartflutter.aviapoint` и SHA256 отпечаток ключа подписи.

2. **Проверка custom scheme без браузера:**  
   Убедиться, что приложение вообще получает диплинк:
   ```bash
   adb shell am start -a android.intent.action.VIEW -c android.intent.category.BROWSABLE -d "aviapoint://payment/success" ru.dartflutter.aviapoint
   ```
   Или для https (если assetlinks уже настроен):
   ```bash
   adb shell am start -a android.intent.action.VIEW -c android.intent.category.BROWSABLE -d "https://avia-point.com/on-the-way/1" ru.dartflutter.aviapoint
   ```

3. **Проверка с браузером/другим приложением:**  
   После настройки assetlinks нужно открывать ссылку именно по тапу из браузера/мессенджера — тогда система предложит «Открыть в приложении» или откроет приложение сразу.

**Требуется:**
- ⚠️ Разместить .well-known/assetlinks.json на сервере avia-point.com (для открытия https-ссылок в приложении на Android)
- ⚠️ При необходимости добавить обработку payment/success и payment/cancel в роутер или WebView
- ⚠️ Протестировать диплинки на реальном устройстве (adb для scheme, тап по ссылке — для App Links)
