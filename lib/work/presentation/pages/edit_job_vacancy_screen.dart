...
class EditJobVacancyScreen extends StatefulWidget {
  ...
  const EditJobVacancyScreen({super.key, @PathParam('id') required this.id});
...
class _EditJobVacancyScreenState extends State<EditJobVacancyScreen> {
  ...
  late final JobVacancyDetailBloc _detailBloc;
  late final JobVacancyEditBloc _editBloc;
  ...
  void initState() {
    super.initState();
    _detailBloc = JobVacancyDetailBloc(repository: getIt<JobsRepository>())..add(JobVacancyDetailEvent.getById(id: widget.id));
    _editBloc = JobVacancyEditBloc(repository: getIt<JobsRepository>());
  ...
  void _fillFromVacancy(JobVacancyEntity v) {
    _titleController.text = v.title;
    _descriptionController.text = v.description ?? '';
    _responsibilitiesController.text = v.responsibilities ?? '';
    _requirementsController.text = v.requirements ?? '';
    _conditionsController.text = v.conditions ?? '';
    _salaryFromController.text = v.salaryFrom?.toString() ?? '';
    _salaryToController.text = v.salaryTo?.toString() ?? '';
    setState(() {
      _selectedContactProfileId = v.contactProfileId;
      _isCompany = !(v.isPrivate ?? false);
      _selectedAddress = v.address;
      _logoPhoto = v.logoUrl != null && v.logoUrl!.isNotEmpty
          ? _PhotoItem(url: v.logoUrl, file: null, bytes: null, isNew: false)
          : null;
      _additionalPhotos = (v.additionalImageUrls).map((url) => _PhotoItem(url: url, file: null, bytes: null, isNew: false)).toList();
    });
    _lastInnLookup = null;
    _checkoCubit.reset();
    _employmentFormController.text = v.employmentForm ?? '';
    _workHoursController.text = v.workHours ?? '';
    _skills
      ..clear()
      ..addAll(v.skills ?? []);
    _currency = v.currency ?? 'RUB';
    _employmentType = v.employmentType;
    _schedule = v.schedule;
    _experienceLevel = v.experienceLevel;
    _educationLevel = v.educationLevel;
    _isGross = v.isGross ?? true;
    _relocationAllowed = v.relocationAllowed ?? false;
    _publish = v.isPublished;
  }
  ...
  Future<void> _submit() async {
    ...
    List<String> additionalImageUrls = _additionalPhotos
        .where((p) => !p.isNew && p.url != null && p.url!.isNotEmpty)
        .map((p) => p.url!)
        .toList();
    final newFiles = _additionalPhotos.where((p) => p.isNew && p.file != null).map((p) => p.file!).toList();
    if (newFiles.isNotEmpty) {
      final uploadResult = await getIt<JobsRepository>().uploadVacancyAdditionalImages(widget.id, newFiles);
      if (!mounted) return;
      uploadResult.fold(
        (_) => ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Не все файлы загружены'), backgroundColor: Colors.orange)),
        (newUrls) => additionalImageUrls = [...additionalImageUrls, ...newUrls],
      );
    }

    _editBloc.add(
      JobVacancyEditEvent.update(
        id: widget.id,
  ...
  Widget build(BuildContext context) {
    return MultiBlocProvider(
      providers: [
        BlocProvider<JobVacancyDetailBloc>.value(value: _detailBloc),
        BlocProvider<JobVacancyEditBloc>.value(value: _editBloc),
      ...
      child: MultiBlocListener(
        listeners: [
          BlocListener<JobVacancyDetailBloc, JobVacancyDetailState>(
            listener: (context, state) {
              state.maybeWhen(
                success: (vacancy) {
                  _fillFromVacancy(vacancy);
                },
                orElse: () {},
              );
            },
            ...
            listener: (context, state) {
              state.maybeWhen(
                updated: (vacancy) {
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Вакансия обновлена'), backgroundColor: Colors.green));
        ...
        child: Scaffold(
          appBar: const CustomAppBar(title: 'Редактировать вакансию', withBack: true),
          ...
          body: BlocBuilder<JobVacancyDetailBloc, JobVacancyDetailState>(
            builder: (context, detailState) {
              ...
              return BlocBuilder<JobVacancyEditBloc, JobVacancyEditState>(
                        ...
                        child: Form(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              _buildSelectRow(
                                label: 'Тип занятости',
                                value: _employmentType != null ? _mapEmploymentType(_employmentType!) : 'Не выбрано',
                                onTap: () => _openEmploymentTypeSheet(context),
                              ),
                              const SizedBox(height: 12),
                              ...
                              Container(
                                ...
                                child: SwitchListTile(
                                  title: Text('Опубликована', style: AppStyles.regular14s.copyWith(color: AppColors.textPrimary)),
                                  subtitle: Text('Снять галочку — снять вакансию с публикации', style: AppStyles.regular12s.copyWith(color: AppColors.textSecondary)),
                                  value: _publish,
