...
class _AirportInfoBottomSheetState extends State<AirportInfoBottomSheet> {
            ...
            child: Column(
              ...
              children: [
                ...
                Stack(
                  ...
                  children: [
                    Text(_airport != null ? _getTitleForAirport(_airport!) : 'Информация об аэропорте', style: AppStyles.bold20s.copyWith(color: Color(0xFF374151))),
                    Positioned(
                      right: 0,
  ...
  void _showUploadVisitorPhotosDialog(BuildContext context, AirportModel airport) async {
    ...
    try {
      ...
      final updatedAirport = await airportService.uploadVisitorPhotos(airportCode: airport.code, photos: images);
                            ...
                            Row(
                              ...
                              children: [
                                ...
                                if (isAuthenticated) ...[
                                  ...
                                  IconButton(
                                    icon: Icon(Icons.delete_outline, color: Colors.red, size: 24),
                                    onPressed: () => _deleteVisitorPhotoFromViewer(mainContext, dialogContext, airport, photos[currentIndex], currentIndex, photos, setState, pageController),
                                    style: IconButton.styleFrom(backgroundColor: Colors.black.withOpacity(0.5), shape: CircleBorder()),
                                    tooltip: 'Удалить',
                                  ),
                                ],
  ...
  Future<void> _deleteVisitorPhoto(BuildContext context, AirportModel airport, String photoUrl) async {
    ...
    try {
      ...
      await airportService.deleteVisitorPhoto(airportCode: airport.code, photoUrl: photoUrl);
      ...
      if (mounted) {
        scaffoldMessenger.showSnackBar(SnackBar(content: Text('Не удалось удалить фотографию: ${e.toString()}'), backgroundColor: Colors.red, duration: Duration(seconds: 3)));
      }
    }
  }
