...
abstract class MarketCategoriesEvent with _$MarketCategoriesEvent {
  ...
  const factory MarketCategoriesEvent.getSubcategories({required String productType, int? parentId, int? mainCategoryId}) = GetSubcategoriesEvent;
}
...
class MarketCategoriesBloc extends Bloc<MarketCategoriesEvent, MarketCategoriesState> {
  ...
  Future<void> _onGetAllCategories(GetAllMarketCategoriesEvent event, Emitter<MarketCategoriesState> emit) async {
    ...
    final result = event.productType == 'parts'
        ? await _repository.getPartsMainCategories() // Для запчастей пока используем main categories
  ...
  Future<void> _onGetSubcategories(GetSubcategoriesEvent event, Emitter<MarketCategoriesState> emit) async {
    ...
    // Для запчастей используем специальный метод
    print('🔵 _onGetSubcategories: productType=${event.productType}, mainCategoryId=${event.mainCategoryId}, parentId=${event.parentId}');
    final result = event.productType == 'parts'
    ...
    result.fold(
      ...
      (categories) {
        ...
        print('🔵 Подкатегории: ${categories.map((c) => 'id=${c.id}, name=${c.name}, parentId=${c.parentId}, partsMainCategoryId=${c.partsMainCategoryId}').join(', ')}');
        ...
        if (currentState is SuccessMarketCategoriesState) {
          ...
          final mainCategories = currentState.categories.where((c) => 
            c.parentId == null && c.partsMainCategoryId == null
          ).toList();
          ...
          final existingSubcategories = currentState.categories.where((c) => 
            c.partsMainCategoryId != null && !newCategoryIds.contains(c.id)
          ).toList();
          
          // Добавляем новые подкатегории
          final allCategories = [...mainCategories, ...existingSubcategories, ...categories];
          print('🔵 Итого категорий после объединения: ${allCategories.length} (главных: ${mainCategories.length}, существующих подкатегорий: ${existingSubcategories.length}, новых подкатегорий: ${categories.length})');
          print('🔵 Все категории после объединения: ${allCategories.map((c) => 'id=${c.id}, name=${c.name}, parentId=${c.parentId}, partsMainCategoryId=${c.partsMainCategoryId}').join(', ')}');
