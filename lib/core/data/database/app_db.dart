...
class AppSettings extends Table {
  ...
  BoolColumn get mixAnswers => boolean().withDefault(const Constant(false))();
  BoolColumn get mixQuestions => boolean().withDefault(const Constant(false))();
  BoolColumn get buttonHint => boolean().withDefault(const Constant(false))();
  ...
  Future<void> saveTestMode({required int certificateTypeId, required String testMode}) async {
    try {
      ...
      if (existing != null) {
        ...
        await into(
          appSettings,
        ).insert(AppSetting(certificateTypeId: certificateTypeId, testMode: testMode, mixAnswers: false, mixQuestions: false, buttonHint: false, selectedCategoryIds: const {}, title: '', image: ''));
      }
