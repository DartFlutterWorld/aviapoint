...
class BlogArticleDetailState with _$BlogArticleDetailState {
  ...
  const factory BlogArticleDetailState.error({
    String? errorFromApi,
...
class BlogArticleDetailBloc extends Bloc<BlogArticleDetailEvent, BlogArticleDetailState> {
  ...
  BlogArticleDetailBloc({required BlogRepository blogRepository})
    : _blogRepository = blogRepository,
      super(const LoadingBlogArticleDetailState()) {
    on<BlogArticleDetailEvent>(
      (event, emit) => event.map(
        getById: (event) => _getById(event, emit),
        reset: (_) => emit(const LoadingBlogArticleDetailState()),
      ),
    );
  }
